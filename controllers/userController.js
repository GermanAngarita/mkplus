...
function signUp( req, res ){
    const user = new User({
        ...
        servicePackAccess: req.body.servicePackAccess,
    })
    User.findOne({email: req.body.email}, (err, validation) =>{
        ...
        user.save((err) => {
            ...
            return res.status(200).send({message:'Se ha creado el usuario', token: service.createToken(user), user:user})
        })
...
function signIn( req, res ){
    ...
    User.findOne({ email: userEmail}, (err, user)=>{
        if(err) return res.status(500).send({message:`No se pudo hacer conexión con el servidor, Por favor confirme al acceso a la siguiente url: https://app.kia.com.co:3001/api/private`, err:err})
...
function upDateUser (req, res){
    ...
    let upDate = req.body
    if(upDate.newPassword && upDate.newPasswordc){
        
        if(upDate.password) bcrypt.genSalt(10, (err, salt)=>{
