...
function getDealers(req, res, next){
    Dealer.aggregate([
        ...
        if(error) return res.status(500).send({msg:'Error al obtener los distribuidores', error:error})
        allDealers = result;
        next();
    })
}

function getSubName(cl){
    let subname='';
...
function getAdvisorName(id){
    let name = ''
    for(let i of allTeam){
        if(i.id == id){
            name =`[${getSubName(i.code_dealer)}] ${i.name.split(' ')[0]? i.name.split(' ')[0]:i.name } ${i.last_name.split(' ')[0]? i.last_name.split(' ')[0]:i.last_name}`;
        }
    }
    if(name){
        return name
...
function getTeamById(req, res, next){
    Teams.aggregate([
        { $group:{
            _id:{ id:"$nid", name:"$name", last_name:"$last_name", code_dealer:"$code_dealer" }
        }},
        { $project:{
            _id:0,
            id:"$_id.id",
...
function kacsGeneral(req, res){
    ...
    for(let i of model){
        let item = new RegExp(i)
        modelPattron.push(item)
    }
        ...
        if(kacs.length>0){
            ...
            Dealer.aggregate([
                ...
                for(let i of kacs){
                    ...
                    data.color.push(
                        colors.getColor(type, dateFrom, dcsi, kacs)
                    )
...
function satisfactionIndFRFT(req, res){
    ...
    if(type==="SA"){
        return res.status(200).send({message:'no aplica'})
    }
    Dcsi.aggregate([
        { $match: { type:type }},
        { $match: { date:{$lte:dateTo} }},
...
function getKacsResult(req, res){
    ...
    Dcsi.aggregate([
        ...
        { $group: {
            ...
            max_point:{ $sum:{ $multiply:[ 10, { $sum:1} ] } }
        }},
        { $group: {
            _id:"$_id.dcsi",
            answers:{ $sum:"$total"},
...
function loyaltyPerDealer(req, res){
    // Incluir type para comercial y cambiar los códigos de pregunta [BQ030, CQ010, CQ020]
    let type = req.body.type;
    let dateFrom = parseInt(req.body.fromDate)
    let dateTo = parseInt(req.body.toDate)
    let group = req.body.group
        ...
        if(loyalty.length>0){
            Dealer.aggregate([
                ...
                for(let i of loyalty){
                    ...
                    data.color.push(colors.getColor(type, dateFrom, 'loyalty', Math.round((total / i.surveys.length * 100)*100)/100))
                }
                res.status(200).send(data)
            ...
            for(let i=0; i<kascDetails.length;i++){
                ...
                for(let j=0; j<kascDetails[i].answers.length;j++){
                    switch(kascDetails[i].answers[j].answer){
                        ...
                        break;
                        case 6:
                        not0 = kascDetails[i].answers[j].total / kascDetails[i].total
                        break;
                        case 5:
                        not1 = kascDetails[i].answers[j].total / kascDetails[i].total
                        break;
                        case 4:
                ...
                data[i].series.push({
                    name:'Insatisfecho',
                    value: not0+not1+not2+not3+not4+not5
                })
                data[i].series.push({
                    name:'Neutral',
                    value: neutral1+neutral2
                })
                data[i].series.push({
                    name:'Satisfecho',
            ...
            for(let i=0; i<kascDetails.length;i++){
                ...
                for(let j=0; j<kascDetails[i].answers.length;j++){
                    switch(kascDetails[i].answers[j].answer){
                        ...
                        break;
                        case 0:
                        not6 = (kascDetails[i].answers[j].total / kascDetails[i].total)
                        break;
                ...
                data[i].series.push({
                    ...
                    value: not0+not1+not2+not3+not4+not5+not6
                })
                data[i].series.push({
                    name:'Neutral',
                    value: neutral1+neutral2
    ...
    Dcsi.aggregate([
        ...
        if(kacs.length>0){
            ...
            Dealer.aggregate([
                ...
                //Total Dealers
                info.dealers = parseInt(data.labels.length)
                //Min y Max
                let dealersSorts = []
        ...
        if(kacs){
            ...
            Dealer.aggregate([
                ...
                if(dealerByGroup){
                    ...
                    for(let i of newData.values){
                        if(i){
                            newData.color.push(colors.getColor(type, dateFrom, 'BQ010', i))
                        } else {
                            newData.color.push(
                                'rgba(255, 255, 255)'
                            )
                    ...
                    if(dealerByGroup){
                        ...
                        for(let i of newData.values){
                            if(i){
                                ...
                                //     )
                                // }
                                newData.color.push(colors.getColor(type, dateFrom, 'loyalty', i))
        ...
        if(frft.length>0){
            Dealer.aggregate([
                ...
                for(let i of data){
                    ...
                    newData.color.push(
                        colors.getColor(type, dateFrom, 'BQ020', i.value)
                    )
                }
                    ...
                    if(dealerByGroup){
                        ...
                        for(let i of newData2.values){
                            if(i){
                                ...
                                //     )
                                // }
                                newData2.color.push(colors.getColor(type, dateFrom, 'BQ020', i))
                            } else {
                                newData2.color.push(
                                    'rgba(255, 255, 255)'
                                )
...
function getRetentionRate(req, res){
    ...
    let dcsi = req.body.dcsi
    let type = req.body.type
    let data = {cl:[], name:[], city:[], labels:[], values:[], pasive:[], npsCountry:[], detractors:[], nps:[], color:[]}
    ...
    Dcsi.aggregate([
        ...
        if(retention.length>0){
            Dealer.aggregate([
                ...
                for(let j of dealer){
                    for(let i=0; i<retention.length; i++){
                        ...
                        if(retention[i].cl == j.cl){
                            retention[i].av = j.av
                            retention[i].name = j.name
                            retention[i].city = j.city
                        }
                    }
                }
                retention.sort((a,b)=>{
                ...
                for(let i=0; i<retention.length; i++){
                    let detractors1 = 0, detractors0 = 0, detractors2 = 0, detractors3 = 0, detractors4 = 0, detractors5 = 0, detractors6 = 0, pasive7 = 0, pasive8 = 0, promoter9 = 0, promoter10 = 0
                    data.labels.push(retention[i].av)
                    data.cl.push(retention[i].cl)
                    data.name.push(retention[i].name)
                    data.city.push(retention[i].city)
                    // Clasificación del Metodo NPS
                    for(let j=0; j<retention[i].answers.length; j++){
                        switch(retention[i].answers[j].answer){
                            case 0:
                                detractors0 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 1:
                                detractors1 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 2:
                                detractors2 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 3:
                                detractors3 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 4:
                                detractors4 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 5:
                                detractors5 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 6:
                                detractors6 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 7:
                                pasive7 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 8:
                                pasive8 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 9:
                                promoter9 = retention[i].answers[j].total / retention[i].surveys
                            break;
                            case 10:
                                promoter10 = retention[i].answers[j].total / retention[i].surveys
                            break;
                        }
                    }
                    // Fin
                    retention[i].promoter = promoter9 + promoter10
                    retention[i].pasive = pasive7 + pasive8
                    retention[i].detractors = detractors0 + detractors1 + detractors2 + detractors3 + detractors4 + detractors5 + detractors6
                    data.values.push( Math.round( retention[i].promoter*10000)/100 )
                    data.pasive.push( Math.round( retention[i].pasive*10000)/100 )
                    data.detractors.push( Math.round( retention[i].detractors*10000)/100 )
                    data.nps.push( Math.round( (retention[i].promoter - retention[i].detractors)*10000 )/100 )
                    // Color 
                    if((retention[i].promoter - retention[i].detractors) > 0.5 || (retention[i].promoter - retention[i].detractors) == 0.5){
                        data.color.push(verde)
                    } else if ((retention[i].promoter - retention[i].detractors) > 0.25 && (retention[i].promoter - retention[i].detractors) < 0.5){
...
function getAvgNPSScore(req, res){
    let type = req.body.type
    let dateFrom = parseInt(req.body.fromDate)
    let dateTo = parseInt(req.body.toDate)
    let dcsi = req.body.dcsi
    Dcsi.aggregate([
        ...
        if(err) return res.status(500).send({msg:'Error al obtener los promedios', err:err})
        if(avg && avg.length>0){
            let data = { surveys:0, promote:0, pasive:0, detractors:0, nps:0 }
            let d0=0, d1=0, d2=0, d3=0, d4=0, d5=0, d6=0
            let p7=0, p8 = 0
            let po9=0, po10 = 0
            for(let i of avg){
                data.surveys += i.surveys;
            }
            for(let i of avg){
                switch(i.answer){
                    
                    case 0:
                        d0 = i.total / data.surveys
                    break;
                    case 1:
                        d1 = i.total / data.surveys
                    break;
                    case 2:
                        d2 = i.total / data.surveys
                    break;
                    case 3:
                        d3 = i.total / data.surveys
                    break;
                    case 4:
                        d4 = i.total / data.surveys
                    break;
                    case 5:
                        d5 = i.total / data.surveys
                    break;
                    case 6:
                        d6 = i.total / data.surveys
                    break;
                    case 7:
                        p7 = i.total / data.surveys
                    break;
                    case 8:
                        p8 = i.total / data.surveys
                    break;
                    case 9:
                        po9 = i.total / data.surveys
                    break;
                    case 10:
                        po10 = i.total / data.surveys
                    break;
                }
            }

            data.promote = Math.round( (po10 + po9)*10000 ) /100
            data.pasive = Math.round( ( p7 + p8)*10000)/100
            if(d0){
                data.detractors = Math.round( (d0 + d1 + d2 + d3 + d4 + d5 + d6)*10000 )/100
            } else {
                data.detractors = Math.round( (d1 + d2 + d3 + d4 + d5 + d6)*10000 )/100
            }
            data.nps = data.promote - data.detractors
            // data.data = avg
            res.status(200).send(data)
...
function getFLCRate(req, res){
    ...
    let dcsi = ["SQ130", "SQ160", "SQ090", "SQ120"]
    Dcsi.aggregate([
        { $match:{ type:'SE' }},
        { $match: { cod_dcsi:{ $in:dcsi}}},
        { $match: { answer: { $nin:[0]}}},
        ...
        if(flcRate){
            for(let i=0; i<flcRate.length; i++){
                switch(flcRate[i].dcsi){
                    ...
                    let a=0; let b=0;
                    for(let j=0; j<flcRate[i].answers.length; j++){
                        if(flcRate[i].answers[j].answer == 9){
                            a = flcRate[i].answers[j].total / flcRate[i].surveys
                        } else if(flcRate[i].answers[j].answer == 10){
            ...
            for(let i = 0; i<flc.length; i++){
                ...
                switch(flc[i].dcsi){
                    ...
                    for(let j=0; j<flc[i].answers.length; j++){
                        if (flc[i].answers[j].answer == 9){
                            d += flc[i].answers[j].total / flc[i].surveys
                        } else if(flc[i].answers[j].answer == 10){
            ...
            for(let i=0; i<npsRetention.length; i++){
                ...
                for(let j=0; j<npsRetention[i].dcsi.length; j++){
                    ...
                    switch(npsRetention[i].dcsi[j].dcsi){
                        ...
                        let retention = 0
                        for(let k=0; k<npsRetention[i].dcsi[j].answers.length; k++){
                            if(npsRetention[i].dcsi[j].answers[k].answer == 9){
                                retention += npsRetention[i].dcsi[j].answers[k].total / npsRetention[i].dcsi[j].surveys
                            } else if(npsRetention[i].dcsi[j].answers[k].answer == 10 ){
                                retention += npsRetention[i].dcsi[j].answers[k].total / npsRetention[i].dcsi[j].surveys
                            }
                        }
            ...
            for(let i=0; i<flcRate.length; i++){
                ...
                for(let j=0; j<flcRate[i].dcsi.length; j++){
                    switch( flcRate[i].dcsi[j].dcsi){
                        ...
                        let a=0; let b=0
                        for(let k=0; k< flcRate[i].dcsi[j].answers.length; k++){
                            if( flcRate[i].dcsi[j].answers[k].answer == 9){
                               a =  Math.round( flcRate[i].dcsi[j].answers[k].total / flcRate[i].dcsi[j].surveys*10000 )/100 
                            } 
                            else if( flcRate[i].dcsi[j].answers[k].answer == 10 ){
                ...
                for(let j=0; j<flcEnhanced[i].dcsi.length; j++){
                    switch(flcEnhanced[i].dcsi[j].dcsi){
                        ...
                        for(let k=0; k<flcEnhanced[i].dcsi[j].answers.length; k++){
                            if(flcEnhanced[i].dcsi[j].answers[k].answer == 9){
                                a = flcEnhanced[i].dcsi[j].answers[k].total / flcEnhanced[i].dcsi[j].surveys
                            }else if(flcEnhanced[i].dcsi[j].answers[k].answer == 10){
...
function getKacsDetails(req, res){
    ...
    Dcsi.aggregate([
        ...
        if(kacs){
            if(dcsi == 'SQ150' || dcsi == 'SQ140' || dcsi == 'SQ130' || dcsi == 'SQ120' || dcsi == 'SQ091'){
                for(let i of kacs){
                    for(let j of i.dcsi){
                        let one=0; let two=0;
                        ...
                        j.value = Math.round( (one / j.surveys)*10000 )/100
                    }
                }
            } else {
                for(let i of kacs){
                    for(let j of i.dcsi){
                        ...
                        j.value = Math.round( total / (j.surveys * 10)*10000 )/100
                    }
                }
            }
            Dealer.aggregate([
                ...
                for(let i of kacs){
                    ...
                    for(let j of i.dcsi){
                        ...
                        data.color.push( colors.getColor('SE', dateFrom, dcsi[0], j.value) )
                    }
                }
                res.status(200).send(data)
            })
...
function getKpiByAdvisor(req, res){
    // let  dealer = new RegExp(req.body.dealer, 'i');
    let dateFrom = parseInt(req.body.fromDate)
    let dateTo = parseInt(req.body.toDate) 
    let dealers = [];
    let advisors = [];
    ...
    for(let i of req.body.advisors){
        advisors.push( new RegExp(i, 'i'))
    }
    let data = { labels:[], name:[], value:[], color:[], surveys:[] }
    
    if(req.body.compute === 'avg'){
        Dcsi.aggregate([
            { $match: { date:{$lte:dateTo} }},
            { $match: { date:{$gte:dateFrom} }},
            { $match:{ type:type }},
            { $match:{ cod_dealer:{ $in:dealers}}},
            { $match:{ cod_dcsi:dcsi}},
            { $match:{ advisorId:{ $in:advisors } }},
            { $match:{ advisorId:{ "$exists": true, "$ne": null }}},
            { $group:{
                _id:{ advisor:"$advisorId", cod_dealer:"$cod_dealer" },
                result:{ $avg:"$answer" },
                surveys:{ $sum:1 }
            }},
            { $project:{
            ...
            if(result){
                console.log(result)
                result.map( (i)=>{
                    data.labels.push(i.advisor);
                    data.name.push(getAdvisorName(i.advisor)? getAdvisorName(i.advisor): getSubName(i.dealer))
                    data.value.push( Math.round( (i.result*10)*100 )/100);
                    data.surveys.push(i.surveys);
                    data.color.push( setColor(Math.round( (i.result*10)*100 )/100, scale) );
                } )
        ...
        Dcsi.aggregate([
            ...
            { $group:{
                _id:{ advisor:"$advisorId", cod_dealer:"$cod_dealer" },
                total:{ $sum:{ $cond:[ { $eq:["$answer", 1 ]}, 1, 0 ] } },
                surveys:{ $sum:1 }
            }},
            { $project:{
                _id:0,
            ...
            if(result){
                ...
                result.map( (i)=>{
                    data.labels.push(i.advisor);
                    data.name.push(getAdvisorName(i.advisor)? getAdvisorName(i.advisor): getSubName(i.dealer))
                    data.value.push( Math.round( (i.result)*10000 )/100);
                    data.surveys.push(i.surveys);
                    data.color.push( setColor( Math.round( (i.result)*10000 )/100, scale) );
                } )
...
module.exports = {
    kacsGeneral,                    //OK
    satisfactionIndKacs,            //ok
    satisfactionIndFRFT,            //ok
    satisfactionIndLoyalty,         //ok
    getKacsResult,                  //OK
    getKacsResultTrimonth,          //OK
    loyaltyPerDealer,               //OK
    getKascDetails,
    getRevisitDetails,
    getRecommendDetails,
